...
public class GetAttributeExpression implements Expression<Object> {
                    ...
                    if (object.getClass().isArray()) {
                        ...
                        if (index < 0 || index >= length) {
                            if (context.isStrictVariables()) {
                                String msg = String.format("Index out of bounds while accessing array with strict variables on, on attribute %s at line %s in file %s.", attributeName, lineNumber, filename);
        ...
        if (object != null && member != null) {
            ...
            if (object == null) {
                ...
                if (node instanceof ContextVariableExpression) {
                    ...
                    String msg = String.format("Root attribute [%s] does not exist or can not be accessed and strict variables is set to true at line %s in file %s.", rootPropertyName, this.lineNumber, this.filename);
                    throw new Exception(msg);
                } else {
                    String msg = String.format("Attempt to get attribute of null object and strict variables is set to true at line %s in file %s.", attributeName, this.lineNumber, this.filename);
                    throw new Exception(msg);
                }

            } else {
                String msg = String.format("Attribute [%s] of [%s] does not exist or can not be accessed and strict variables is set to true at line %s in file %s.",attributeName, object.getClass().getName(), this.lineNumber, this.filename);
                throw new Exception(msg);
            }
        }
        return result;
     ...
     * @throws Exception
     */
    private Object[] getArgumentValues(Template self, EvaluationContext context) throws Exception {
